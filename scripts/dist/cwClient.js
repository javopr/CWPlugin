...
export async function resolveCompanyInfo(fqdn, companyId) {
    ...
    try {
        ...
        throw new CwApiError("NETWORK_ERROR", `No se pudo contactar ${fqdn}. Verifica el FQDN. (${err.message})`);
    }
    if (!res.ok) {
        throw new CwApiError("NETWORK_ERROR", `companyinfo respondio ${res.status} para company "${companyId}". Verifica el FQDN y el Company ID.`, res.status);
    }
...
function buildUrl(apiBase, opts) {
...
async function rawRequest(config, secrets, opts) {
    ...
    try {
        ...
        throw new CwApiError("NETWORK_ERROR", `Fallo de red llamando a ConnectWise: ${err.message}`);
    }
}
async function toApiError(res) {
    ...
    if (res.status === 401) {
        ...
        if (message.toLowerCase().includes("clientid")) {
            return new CwApiError("AUTH_MISSING_CLIENTID", "El header clientId parece invalido o falta. Verifica el Client ID registrado en developer.connectwise.com.", 401, details);
        }
        return new CwApiError("AUTH_INVALID", "Las credenciales de ConnectWise no son validas o expiraron. Ejecuta /cw-install para reconfigurar.", 401, details);
    }
    if (res.status === 404) {
    ...
    return new CwApiError("UNKNOWN_ERROR", `ConnectWise respondio con un error inesperado (HTTP ${res.status}).`, res.status, details);
}
/** Authenticated request with one retry on 429 honoring Retry-After. */
export async function request(config, secrets, opts) {
...
export async function requestAllPages(config, secrets, opts, pageSize = 1000) {
    const results = [];
    let page = 1;
    ...
    while (true) {
        const pageResults = await request(config, secrets, {
            ...opts,
        ...
        if (page > MAX_PAGES_SAFETY_CAP) {
            throw new CwApiError("UNKNOWN_ERROR", `Se alcanzo el limite de seguridad de paginacion (${MAX_PAGES_SAFETY_CAP} paginas de ${pageSize}) en ${opts.path}. Hay mas datos de los esperados; acota la consulta.`);
        }
    }
    return results;
}
export async function testConnection(config, secrets) {
