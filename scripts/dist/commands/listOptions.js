...
export async function listWorkRoles(ticketId) {
    const { config, secrets } = loadContext();
    if (ticketId !== undefined) {
        const { raw } = await resolveTicket(config, secrets, ticketId);
        const locationId = raw.location?.id;
        if (locationId !== undefined) {
            const locationRoles = await requestAllPages(config, secrets, {
